...
import { abiTypeToSelectorString, canonicalizeTupleString, isTupleString, isTupleArrayType, parseTupleToStructuredArray, resolveTupleToAbiType, validateTupleInnerTypes, } from './utils/tupleParser.js';
// ------------------------------------------------------------------
// Transform
// ------------------------------------------------------------------
...
`);
        ...
        const tupleTypeNames = new Map();
        for (const fn of abiObj.functions) {
            const allParams = [
                ...fn.inputs.map((p) => ({ ...p, kind: 'input' })),
                ...fn.outputs.map((p) => ({ ...p, kind: 'output' })),
            ];
    ...
    // ------------------------------------------------------------
    buildAbiTsFile(className, abiObj) {
        const lines = [];
        const formatTypeRef = (type) => {
            ...
            if (AbiTypeToStr[type] !== undefined) {
        ...
        // Imports
        lines.push(`import { ABIDataTypes, BitcoinAbiTypes, OP_NET_ABI } from 'opnet';`);
        lines.push('');
        // Events array
        ...
        for (const fn of abiObj.functions) {
            // Build inputs
            const inputsStr = fn.inputs.length === 0
                ? '[]'
                : `[${fn.inputs.map((i) => `{ name: '${i.name}', type: ${formatTypeRef(i.type)} }`).join(', ')}]`;
            // Build outputs
            const outputsStr = fn.outputs.length === 0
                ? '[]'
    ...
    buildExecuteMethod(_className, methods) {
        ...
        for (const m of methods) {
            ...
            if (m.selectorOverride) {
                ...
                logger.debugBright(`Found function ${sig} -> ${selectorNum} [override] (${m.declaration.name.text})`);
            }
            else {
            ...
            if (m.onlyOwner) {
                bodyLines.push(`if (selector == ${selectorNum}) { this.onlyOwner(calldata); ${callLine} }`);
            }
            else {
    ...
    visitMethodDeclaration(node) {
        ...
        if (methodInfo) {
            ...
            if (methodInfo.isView && methodInfo.isPayable) {
                throw new Error(`Compile error: method "${methodInfo.methodName}" in class "${this.currentClassName}" ` +
            ...
            if (existing) {
                ...
                if (methodInfo.isView)
                    existing.isView = true;
                if (methodInfo.isPayable)
                    existing.isPayable = true;
                if (methodInfo.onlyOwner)
                    existing.onlyOwner = true;
                ...
                if (existing.isView && existing.isPayable) {
                    throw new Error(`Compile error: method "${existing.methodName}" in class "${this.currentClassName}" ` +
                        `cannot be both @view and @payable. A view method is read-only and cannot accept payment.`);
                }
            }
            else {
    ...
    mapToAbiDataType(str) {
        ...
        if (isTupleString(str)) {
            ...
            if (invalid.length > 0) {
                logger.error(`Custom tuple "${str}" contains invalid inner types: ${invalid.join(', ')}`);
            }
